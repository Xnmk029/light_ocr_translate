...
1. 按下全局快捷键（默认：`Ctrl+Alt+D`），屏幕冻结变暗。
2. 拖动鼠标左键选择需要翻译的屏幕区域。
3. 松开鼠标——选中的图片区域立即钉在遮罩上。
4. 后台 OCR 与 LLM 翻译完成后，译文无缝替换原文（流式模式下逐行浮现）。
5. **视图模式**：点击工具条或按 `1`(译文) `2`(原文) `3`(上下对照)。
6. **钉住**：按 `P` 或点击"钉住"按钮——转为悬浮置顶小窗，跨截图存活。
7. **保存**：按 `S` 或点击"保存"——同时落盘原图与译文两张 PNG。
8. 退出：`Esc` / 右键任意位置 / 点击遮罩空白处。
...
从 [Releases](https://github.com/Xnmk029/light_ocr_translate/releases) 下载 x64 免安装压缩包，解压到任意目录，双击 `LightOcrTranslate.exe` 即可运行。

v0.3.0 起内置 PP-OCR ONNX 模型（Apache-2.0 许可），无需额外下载模型文件，开箱即用。无需安装、无需管理员权限。
